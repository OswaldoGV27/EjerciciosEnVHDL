...
architecture behaviour of LCD_comp is
    ...
    type state is (LCD_INI, D_OFF, LCD_CLR, D_ON, ENTRY, ASK1, MSG1, MSG2, L1, L2);
...
begin
            ...
            if (rising_edge(clkdly)) then
                case current_state is
                    ...
                    when MSG2 =>  -- Estado que imprime el mensaje en la linea 2 del LCD
                        if contador2 < 15 then             -- Contador para extraer el mensaje
                            Mensaje('1',contador2,Datos);  -- Procedimiento para imprimir mensaje en el LCD
                            if contador < 2 then           -- Generacion de 2 pulsos para este comando
                                EN := not EN;              -- Negacion de la variable, para generar el pulso
                                E <= EN;                   -- Cambio de valor para EN
                                contador := contador + 1;  -- Incremento del contador una vez
    ...
    Clock: process(next_state, countdly, clk, clkdly, clr) begin
        if clr = '0' then                  -- Si hay un reset
            current_state <= LCD_CLR;       -- Que inicie con el estado inicializacion
        elsif (rising_edge(clk)) then  -- Control del reloj (50MHz))
